...
class Net(nn.Module):
	...
	def __init__(self,
		...
		self.aux_heads_serial = nn.ModuleList([
			AUXheadHR(in_channels=64, mid_channels=128, out_channels=n_classes, upsample=1),
			AUXheadHR(in_channels=128, mid_channels=320, out_channels=n_classes, upsample=1),
			AUXheadHR(in_channels=320, mid_channels=512, out_channels=n_classes, upsample=1),
			AUXheadHR(in_channels=512, mid_channels=512, out_channels=n_classes, upsample=1),
		])
...
# 		encoder = self.encoder(x, organs)
		...
		for i in range(len(encoder)):
			output['aux_pvt_{}'.format(i)] = self.aux_heads_serial[i](encoder[i])
