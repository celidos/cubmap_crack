...
class SegformerMAEDecoder(nn.Module):
    def __init__(
            self,
...
class Net(nn.Module):
    ...
    def __init__( self,):
        ...
        self.decoder_mae = SegformerMAEDecoder(
            encoder_dim = encoder_dim,
            decoder_dim = 192*4,
        )
        self.logit = nn.Sequential(
            nn.Conv2d(512, 6, kernel_size=1, padding=0),
        )
        self.decoder_shuffle_mae = nn.PixelShuffle(2)
        self.logit_mae = nn.Sequential(
            nn.Conv2d(192, 3, kernel_size=1, padding=0),
        )
        self.aux = nn.ModuleList([
            nn.Sequential(*[
                nn.Conv2d(encoder_dim[i], 6, kernel_size=1, padding=0),
                MixUpSample(scale_factor=2**(i + 2))
            ]) for i in range(4)
        ])

    def forward_mae(self, batch):
        ...
        B,C,H,W = x.shape
        enc = self.encoder.forward_mae_encoder(x, mask)
        #print([f.shape for f in encoder])

        x, dec = self.decoder_mae(enc)
        x = self.decoder_shuffle_mae(x)
        logit = self.logit_mae(x)
    
        #print(logit.shape)

        output = {}
        output['mae_logit'] = logit
    ...
    def forward(self, batch):
        ...
        for i in range(4):
            output['aux%d_logits' % i] = self.aux[i](encoder[i])
