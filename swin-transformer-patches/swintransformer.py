...
class Net(nn.Module):
    ...
    def __init__(self, cfg, arch='swin_tiny_patch4_window7_224'):
        super(Net, self).__init__()
        self.output_type = cfg['output_type'] if 'output_type' in cfg else ['probability']
        ...
        print('loading Arch {}'.format(arch))
        print('')
        print('Params:')
        print({**cfg['basic']['swin'], **cfg[self.arch]['swin'],
...
#             nn.Conv2d(256, cfg['num_classes'], kernel_size=1, padding=0) for i in range(4)
#         ])
        self.aux = nn.ModuleList([
            AUXhead(in_channels=256, mid_channels=256, out_channels=cfg['num_classes'], upsample=4) for i in range(4)
        ])
...
#                 output['aux%d_loss'%i] = criterion_aux_loss(self.aux[i](decoder[i]),batch['mask'])
        ...
        if 'logits' in self.output_type:
            output['out_logits'] = logit
            for i in range(4):
                output['aux%d_logits' % i] = self.aux[i](decoder[i])

        if 'probability' in self.output_type:
            output['probability'] = F.softmax(logit, dim=1)
