...
class CoaT(nn.Module):
    ...
    def no_weight_decay(self):
        return {'cls_token1', 'cls_token2', 'cls_token3', 'cls_token4', 
                'org_tokens1', 'org_tokens2', 'org_tokens3', 'org_tokens4'}
...
#         print('xxxx', x.shape)
    ...
    def forward_mae(self, x0, organs, mask):
        B = x0.shape[0]
...
#         print('AFTER FIRST STAGE X SHAPE:', x.shape)

        # PATCH SIZE = 4

        mask = F.interpolate(mask, size=(H1, W1), mode='nearest')
#         print('mask.shape', mask)
#         plt.imshow(mask[0][0])
#         plt.show()
#         print('mask', mask.shape)
        NN, LL, _ = x1.size()
        mask_tokens = self.mask_token.expand(NN, LL, -1)
#         print('mask tokens', mask_tokens.shape)
        w = mask.flatten(1).unsqueeze(-1).type_as(mask_tokens)

#         print('w.shape', w.shape)

        x1 = x1 * (1.0 - w) + mask_tokens * w
...
# 		x4_nocls = self.remove_cls(x4)
        ...
        output = dict()
        output['serial'] = [x1_nocls,x2_nocls,x3_nocls,x4_nocls]
        ...
        if self.parallel_depth > 0:
            ...
            output['parallel'] = [x1_nocls,x2_nocls,x3_nocls,x4_nocls]
            output['mae_mask'] = mask
        return output
