...
#         )
        self.decoder = nn.Sequential(
            nn.Conv2d(320, 768, kernel_size=1),
            nn.ReLU(),
            nn.Dropout(p=0.1),
            nn.Conv2d(768, 4*(4**2)*3, kernel_size=1),
        )
        self.decoder_shuffle = nn.PixelShuffle(8)
...
# 		output['probability'] = probability_from_logit
    ...
    def forward_mae(self, batch):
        ...
        B, C, H, W = x.shape
        encoder = self.encoder.forward_mae(x, organs, mask)
        mae_mask = encoder['mae_mask']
        
        pred = self.decoder(encoder['parallel'][-1])
        logit = self.decoder_shuffle(pred)
...
#         print('logit', logit.shape)
        ...
        mask_loss = F.interpolate(mae_mask, size=logit.shape[-2:], mode='nearest')
        image_scales = F.interpolate(batch['image'], size=logit.shape[-2:], mode='area')
        loss = ((logit - image_scales).abs() * mask_loss).sum() / mask_loss.sum()
