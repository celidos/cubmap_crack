...
# 		v_img = v[:,:,1:,:]                                                              # Shape: [B, h, H*W, Ch].
		...
		q_img = q[:,:,2:,:]                                                              # Shape: [B, h, H*W, Ch].
		v_img = v[:,:,2:,:]                                                              # Shape: [B, h, H*W, Ch].
...
class FactorAtt_ConvRelPosEnc(nn.Module):
	...
	def forward(self, x, size):
		...
		# Merge and reshape.
# 		print('factor_att.shape', factor_att.shape, ' crpe.shape', crpe.shape)
...
class ConvPosEnc(nn.Module):
	...
	def forward(self, x, size):
		...
		cls_token, org_token, img_tokens = x[:, :1], x[:, 1:2], x[:, 2:]                                       # Shape: [B, 1, C], [B, H*W, C].
		...
		x = torch.cat((cls_token, org_token, x), dim=1)
...
class ParallelBlock(nn.Module):
    ...
    def interpolate(self, x, output_size, size):
        ...
        out = torch.cat((cls_token, org_token, img_tokens), dim=1)
...
class CoaT(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, embed_dims=[0, 0, 0, 0],
        ...
        # ORGAN tokens.
        # 5 == number of organs
        self.org_tokens1 = nn.Parameter(torch.zeros(5, 1, embed_dims[0]))
        self.org_tokens2 = nn.Parameter(torch.zeros(5, 1, embed_dims[1]))
        self.org_tokens3 = nn.Parameter(torch.zeros(5, 1, embed_dims[2]))
        self.org_tokens4 = nn.Parameter(torch.zeros(5, 1, embed_dims[3]))
        self.org_tokens5 = nn.Parameter(torch.zeros(5, 1, embed_dims[4]))
        ...
        # Serial blocks 5. (CUSTOM INCREASED DEPTH)
        ...
        trunc_normal_(self.org_tokens1, std=.02)
        trunc_normal_(self.org_tokens2, std=.02)
        trunc_normal_(self.org_tokens3, std=.02)
        trunc_normal_(self.org_tokens4, std=.02)
        trunc_normal_(self.org_tokens5, std=.02)
    ...
    def no_weight_decay(self):
        return {'cls_token1', 'cls_token2', 'cls_token3', 'cls_token4', 'cls_token5'
                'org_tokens1', 'org_tokens2', 'org_tokens3', 'org_tokens4', 'org_tokens5'}
    ...
    def insert_cls_and_org(self, x, cls_token, organ_tokens, organs):
        """ Insert CLS and ORGAN token. THIS FUNCTION IS CUSTOM!!!!"""
        cls_tokens = cls_token.expand(x.shape[0], -1, -1)
        org_tokens = organ_tokens[organs]
#         print('oooo', org_tokens.shape)
#         print('cccc', cls_tokens.shape)
#         print('xxxx', x.shape)
        
        
        x = torch.cat((cls_tokens, org_tokens, x), dim=1)
        return x

    def remove_cls_and_org(self, x):
        """ Remove CLS and ORGAN token. THIS FUNCTION IS CUSTOM!!!!"""
        return x[:, 2:, :]
    ...
    def forward(self, x0, organs):
        B = x0.shape[0]

        
        # Serial blocks 1.
        x1, (H1, W1) = self.patch_embed1(x0)
#         print('here!1', x1.shape)
        cls = self.cls_token1#torch.zeros_like(self.cls_token1)#self.cls_token1
# 		x1 = self.insert_cls(x1, cls)
        x1 = self.insert_cls_and_org(x1, cls, self.org_tokens1, organs)
#         print('here!2', x1.shape)
        for blk in self.serial_blocks1:
            x1 = blk(x1, size=(H1, W1))
#         print('here!3')
# 		x1_nocls = self.remove_cls(x1)
        x1_nocls = self.remove_cls_and_org(x1)
...
# 		x2 = self.insert_cls(x2,cls)
        x2 = self.insert_cls_and_org(x2, cls, self.org_tokens2, organs)

        for blk in self.serial_blocks2:
            x2 = blk(x2, size=(H2, W2))
# 		x2_nocls = self.remove_cls(x2)
        x2_nocls = self.remove_cls_and_org(x2)
...
# 		x3 = self.insert_cls(x3, cls)
        x3 = self.insert_cls_and_org(x3, cls, self.org_tokens3, organs)

        for blk in self.serial_blocks3:
            x3 = blk(x3, size=(H3, W3))
# 		x3_nocls = self.remove_cls(x3)
        x3_nocls = self.remove_cls_and_org(x3)
...
# 		x4_nocls = self.remove_cls(x4)
        ...
        if self.parallel_depth > 0:
            ...
            output['parallel'] = [x1_nocls,x2_nocls,x3_nocls,x4_nocls,x5_nocls]
        return output
...
class coat_parallel_small_depth5(CoaT):
	def __init__(self, **kwargs):
		super(coat_parallel_small_depth5, self).__init__(
